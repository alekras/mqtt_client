...
connect_0({Id, connect} = _X, [Publisher] = _Conns) -> {?ST"connect_0 = " ++ atom_to_list(Id) ++ "."?ET, timeout, 100, fun() ->
...
connect_1({Id, connect} = _X, [Publisher] = _Conns) -> {?ST"connect_1 = " ++ atom_to_list(Id) ++ "."?ET, timeout, 100, fun() ->
	register(test_result, self()),
...
connect_2({Id, connect}, [Publisher]) -> {?ST"connect_2 = " ++ atom_to_list(Id) ++ "."?ET, timeout, 100, fun() ->
	register(test_result, self()),
...
connect_3({Id, connect}, [Publisher]) -> {?ST"connect_3 = " ++ atom_to_list(Id) ++ "."?ET, timeout, 100, fun() ->
	register(test_result, self()),
...
connect_4({Id, connect}, [Publisher]) -> {?ST"connect_4 = " ++ atom_to_list(Id) ++ "."?ET, timeout, 100, fun() ->
	register(test_result, self()),
...
connect_5({Id, connect} = _X, [Publisher] = _Conns) -> {?ST"connect_5 = " ++ atom_to_list(Id) ++ "."?ET, timeout, 100, fun() ->
	register(test_result, self()),
...
connect_6({Id, connect} = _X, [Publisher] = _Conns) -> {?ST"connect_6 = " ++ atom_to_list(Id) ++ "."?ET, timeout, 100, fun() ->
	register(test_result, self()),
...
keep_alive(_, Conn) -> {?ST"keep alive test."?ET, timeout, 15, fun() ->	
	timer:sleep(1500),
