...
session_1({1, session} = _X, [Publisher, Subscriber] = Conns) -> {timeout, 100, fun() ->
  ?debug_Fmt("::test:: session_1 : ~p ~p", [_X, Conns]),
...
session_1({2, session} = _X, [Publisher, Subscriber] = Conns) -> {timeout, 100, fun() ->
  ?debug_Fmt("::test:: session_1 : ~p ~p", [_X, Conns]),
	register(test_result, self()),
...
session_1({3, session} = _X, [Publisher, Subscriber] = Conns) -> {timeout, 100, fun() ->
  ?debug_Fmt("::test:: session_1 : ~p ~p", [_X, Conns]),
	register(test_result, self()),
...
session_1({4, session} = _X, [Publisher, Subscriber] = Conns) -> {timeout, 100, fun() ->
  ?debug_Fmt("::test:: session_1 : ~p ~p", [_X, Conns]),
	register(test_result, self()),
